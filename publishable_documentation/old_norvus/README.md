...
## Overview
This repository provides a unified, longitudinal data network linking Medicare Part B physicians, Part D prescribing behaviors, hospital structural characteristics, and clinical quality metrics (HCAHPS, MIPS, ASCQR). 
...
## Order of Scripts
# Code locating in cleaningCode. Run the scripts within these subfolders:
1) cms
2) hcahps
3) mips
4) final_cleaning

the initialize.do script, located at the same level as the above four subfolders within cleaningCode, is necessary to run all scripts. 
...
keep if cms_state == 33 // e.g., filter for New York